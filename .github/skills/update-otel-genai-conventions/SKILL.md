...
A standing **upstream-scan tracking PR** (one carrying the `# meai-otel-genai-producer:state:begin` block) is the exception: it is the durable scan record, not a blocking duplicate. When the preflight surfaces it, continue rather than stopping -- the maintaining workflow owns how that PR is created and incremented.
