...
# live in the producer's PR body state block, never in the branch (so the one PR refreshes
# forever as upstream advances). The state block's marker is a producer-internal detail --
# the manager neither emits nor knows it.
target="$(jq -cn \
