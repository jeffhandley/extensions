...
# The maintained PR always targets main (see the producer's create-pull-request
...
BASE_BRANCH="main"
# The state block's delimiters are hard-coded to this workflow's name -- a producer-internal
# concern the manager neither knows nor supplies. The block is delimited by whole yaml-comment
# lines `# ${STATE_MARKER}:state:begin` / `:state:end`.
STATE_MARKER="meai-otel-genai-producer"
upstream_sha=""
