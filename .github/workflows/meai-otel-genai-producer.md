  ...
  - name: Validate agent output identity
    run: |
      ...
      while IFS= read -r i; do
        ...
        # is delimited by whole `# meai-otel-genai-producer:state:begin` / `:state:end` comment
        ...
        blk=$(printf '%s' "$body" | awk '{t=$0;sub(/\r$/,"",t);gsub(/^[[:space:]]+|[[:space:]]+$/,"",t)} t=="# meai-otel-genai-producer:state:begin"{inb=1;buf=$0 ORS;next} inb{buf=buf $0 ORS; if(t=="# meai-otel-genai-producer:state:end"){inb=0;last=buf}} END{if(inb)last=buf; printf "%s", last}')
        printf '%s\n' "$body" | awk '{t=$0;sub(/\r$/,"",t);gsub(/^[[:space:]]+|[[:space:]]+$/,"",t)} t=="# meai-otel-genai-producer:state:begin"{f=1} END{exit !f}' || miss="$miss begin-marker"
        printf '%s\n' "$body" | awk '{t=$0;sub(/\r$/,"",t);gsub(/^[[:space:]]+|[[:space:]]+$/,"",t)} t=="# meai-otel-genai-producer:state:end"{f=1} END{exit !f}' || miss="$miss end-marker"
...
- **`target.json`** -- the resolved scan target and PR discovery:
  ...
  - `classification` -- one of:
    - `ours` -- an open `automation`+`area-ai` PR on `pr_branch` that already carries
      our `meai-otel-genai-producer:state` block. Maintain it.
...
yourself once, locate its `# meai-otel-genai-producer:state:begin` .. `:state:end`
...
- **Fresh path** (merged-but-behind, closed, or no PR): stay on the commit that was
  ...
  **not** run `git fetch`, `git reset`, `git rebase`, or `git merge`. Make your edits,
  ...
  do **not** invent a placeholder file or an empty commit. Continue with the branch-ref
  and `create-pull-request` steps below; the safe output is configured with
  `allow-empty: true` so the draft tracking PR can be created with the PR title and
  body only.
...
```yaml
# meai-otel-genai-producer:state:begin
...
# meai-otel-genai-producer:state:end
```
