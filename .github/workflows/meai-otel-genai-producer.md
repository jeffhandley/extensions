...
- `classification` = `ours` -> compare `pr_recorded_sha` to `upstream_sha`
  and pick the row below. When you need to fetch, check out, or push the maintained
  PR branch, use `target.json.pr_branch` (the actual branch), not
  `target.json.desired_branch` (the canonical branch).
