...
# dispatch anywhere (e.g. for testing in a fork). Also run when a write-permission
# reviewer submits a review on the maintained **draft** PR; once the PR leaves draft
# (Ready for Review) those review events no longer trigger an automatic response.
if: ${{ github.event_name == 'workflow_dispatch' || (github.event_name == 'pull_request_review' && github.event.pull_request.draft) || github.event.repository.fork == false }}
