...
`<!-- otel-genai-tracking:begin -->` block in the body (the skill-owned title, which
...
| **behind** the upstream SHA | open **draft** | **Incremental update.** Re-analyze against `main` plus what the branch already integrates; push one batch of commit(s) to the PR's existing head branch; refresh the PR body/tracking block; comment summarizing the delta. |
...
`<!-- otel-genai-tracking:begin -->` state block that **ends** the body. Do not
...
run. On an incremental update, the skill advances `Upstream-Scan-Ref` /
`Upstream-Scan-Date` (and the dependency/implemented-version fields if they moved),
per its "Refreshing the tracking PR" procedure.
