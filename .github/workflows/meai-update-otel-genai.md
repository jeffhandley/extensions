...
says nothing about whether an active PR exists. From any matching PR, parse the
...
| **behind** the upstream SHA | open **draft** | **Incremental update.** Re-analyze against `main` plus what the branch already integrates; push one batch of commit(s) to the PR's existing head branch; refresh the PR body/tracking block. Comment summarizing the delta **only** when the new commits integrate or newly document a convention change; if they carry **zero** convention changes (only chore/dependency/CI/doc commits), advance the scan-ref via the body update with **no** comment. |
...
- Use `add-comment` for incremental summaries, advisory notes on behind non-draft
  PRs, advisory notes on a still-open prior published-version PR that newer changes
  are accumulating, and the release-published note (Step 6). At most **one** comment
  per run. **Skip the incremental comment entirely** when the scan only advances over
  upstream commits that carry **zero** convention changes (chore/dependency/CI/doc):
  silently advance the tracking-block scan-ref via `update-pull-request` with no
  comment, since there is nothing convention-relevant to report.
